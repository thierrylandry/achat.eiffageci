...
@section('content')
    <h2>LES FOURNISSEURS - {{isset($fournisseur)? 'MODIFIER FOURNISSEUR':'AJOUTER FOURNISSEUR'}}<a href="{{route('ajouter_fournisseur')}}" class="btn btn-default  pull-right"><i class="fa fa-plus-circle" aria-hidden="true"></i> Ajouter</a><a href="{{route('lister_fournisseurs')}}" class="btn btn-default pull-right"><i class="fa fa-list" aria-hidden="true"></i> Lister</a></h2>
        ...
        <form role="form" id="FormRegister" class="bucket-form" method="post" action="{{route('update_fournisseur')}}">
                        ...
                        <div class="col-sm-6 col-lg-push-3">
                            ...
                            <div id="contacts">
                                    ...
                                    <div class="form-group">
                                        <select type="text" name="type_c[]" class="type_c form-control input-field">
                                            @foreach(\App\Metier\Json\Contact::$typeListe as $key => $typeliste)
                                              @if($contact->type_c==$key)
                                                    <option value="{{ $key }}" selected>{{ $typeliste }}</option>
                                                  @else
                                ...
                                <div class="col-lg-3 col-md-2 col-sm-7 col-xs-7">
                                    <div class="form-group">
                                        <div class="form-line">
                                            <input type="text" name="valeur_c[]" class="valeur_c form-control" placeholder="Valeur" value="{{isset($contact)? $contact->valeur_c:''}}">
                                        </div>
                                    </div>
                                </div>
                                    @endforeach
